...
with open("tests/operations.json", encoding="utf-8") as file:
    text = json.load(file)
test_text = 'tests/operations.json'
def test_open_file():
  assert utils.open_file(test_text) == text

def test_chek_state_and_sorted():
    assert utils.chek_state_and_sorted([{}]) == []
    assert utils.chek_state_and_sorted(date) == test_date
